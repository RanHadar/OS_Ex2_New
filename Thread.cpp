...
Thread::Thread(int id, void (*f)(void),int stackSize, int priority): threadID(id),priority(priority),state(READY)
{
    ...
    pc = (address_t) f;
    sigsetjmp(env, 1); //todo - understand wtf
    (env->__jmpbuf)[JB_SP] = translate_address(sp);
    (env->__jmpbuf)[JB_PC] = translate_address(pc);
    sigemptyset(&env->__saved_mask);

    //todo - quantum run time
}
